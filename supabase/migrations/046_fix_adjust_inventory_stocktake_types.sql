-- 046_fix_adjust_inventory_stocktake_types.sql
-- 修复：adjust_inventory 函数类型校验缺少 stocktake_in / stocktake_out
-- 根因：043_stocktakes.sql 只扩展了 inventory_transactions 表 CHECK 约束，
--       未同步更新 adjust_inventory 函数内的 p_type 白名单，导致盘点审核报 INVALID_INVENTORY_TYPE。
-- 修复：重新创建函数，类型白名单加入 stocktake_in / stocktake_out。
...
declare
  ...
  v_after numeric;
begin
  if p_type not in ('purchase_in','sales_out','transfer_out','transfer_in','adjustment','after_sales_in','loss','other','stocktake_in','stocktake_out') then
